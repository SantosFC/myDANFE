...
if pagina == "Importar Nota":
    ...
    chave_nfe = (
        ...
        if (m := RE_CHAVE_NFE.search(texto_nfe or ""))
        else None
    )
    chave_prod = (
        re.sub(r"\D", "", m.group(1))
        if (m := RE_CHAVE_PROD.search(texto_prod or ""))
        else None
    )
    ...
    if chave_nfe and chave_prod:
        if chave_nfe == chave_prod:
            ...
            st.error(
                "❌ As chaves dos dois campos **não coincidem**. Verifique se colou abas da mesma nota."
            )
    elif chave_nfe:
        st.caption(
            f"🔑 Chave identificada no campo 1: `{chave_nfe[:10]}...{chave_nfe[-6:]}`"
        )
    elif chave_prod:
        st.caption(
            f"🔑 Chave identificada no campo 2: `{chave_prod[:10]}...{chave_prod[-6:]}`"
        )
    ...
    if st.button("Processar", type="primary", disabled=not (texto_nfe and texto_prod)):
        with st.spinner("Processando nota..."):
            try:
                ...
                st.toast(
                    f"Nota lida com sucesso — {len(itens)} itens encontrados.",
                    icon="✅",
                )
    ...
    if "preview_cab" in st.session_state and not st.session_state.get("importado"):
        ...
        if ja_importada:
            st.warning(
                "⚠️ Esta nota já foi importada anteriormente. Não é possível importar novamente."
            )
        else:
            st.info(
                f"📋 {len(itens)} itens prontos para importação. Confira os dados abaixo e clique em **Confirmar e Salvar**."
            )
        ...
        if st.button("Confirmar e Salvar", type="primary", disabled=ja_importada):
            with st.spinner("Salvando nota no banco de dados..."):
                try:
                    ...
                    st.success(
                        f"✅ Nota importada com sucesso! {len(itens)} itens salvos."
                    )
                    st.balloons()
                except Exception as exc:
...
# ════════════════════════════════════════════════════════
    ...
    st.caption(
        "Selecione um produto pelo nome original da nota e defina um nome mais legível. A alteração se aplica a todos os registros com essa descrição."
    )
    ...
    if st.button(
        "Salvar",
        type="primary",
        disabled=(novo_nome.strip() == selecionado or not novo_nome.strip()),
    ):
...
# ════════════════════════════════════════════════════════
    ...
    if not rows:
        st.info(
            "Nenhum dado ainda. Vá em **Importar Nota** para adicionar sua primeira nota."
        )
        st.stop()
    ...
    if infl.empty:
        st.info(
            "Dados insuficientes para calcular variação mensal. Importe notas de pelo menos 2 meses diferentes."
        )
    else:
    ...
    with col_a:
        ...
        fig2 = px.bar(
            ...
            y="descricao_nota",
            orientation="h",
            labels={"gasto_total": "Gasto (R$)", "descricao_nota": ""},
            height=450,
        )
